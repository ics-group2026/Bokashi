...
import 'package:bokashi/features/auth/widgets/code_picker_widget.dart';
...
import 'package:bokashi/features/splash/domain/models/config_model.dart' as config;
import 'package:bokashi/helper/country_code_helper.dart';
import 'package:bokashi/helper/route_healper.dart';
import 'package:bokashi/helper/velidate_check.dart';
...
import 'package:bokashi/common/basewidget/success_dialog_widget.dart';
...
class _AddNewAddressScreenState extends State<AddNewAddressScreen> {
  ...
  void _checkPermission(Function callback, BuildContext context) async {
    if (_permissionInProgress) return;
    _permissionInProgress = true;
    try {
      LocationPermission permission = await Geolocator.checkPermission();
      
      // Only request if denied, not if already granted
      if (permission == LocationPermission.denied) {
        try {
          permission = await Geolocator.requestPermission();
        } on PlatformException catch (e) {
          // Handle permission request in progress error
          if (e.code == 'permissionRequestInProgress' || 
              e.message?.contains('permissionRequestInProgress') == true) {
            // Wait a bit longer and retry
            await Future.delayed(const Duration(milliseconds: 1000));
            permission = await Geolocator.checkPermission();
            // If still denied after waiting, don't request again to avoid loop
            if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
        // Show dialog but don't request permission again immediately
        if (context.mounted) {
          showDialog(
            context: context,
            builder: (context) => AlertDialog(
              content: SuccessDialog(
                ...
                title: '',
                description: getTranslated('you_denied', context) ?? 'Location permission is required'
              )
            ),
          );
        }
        ...
        // Permission granted, proceed with callback
        callback();
      }
    } catch (e) {
      // Handle any other errors
