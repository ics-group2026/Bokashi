...
import 'package:bokashi/features/location/domain/models/place_details_model.dart';
import 'package:bokashi/features/location/domain/models/prediction_model.dart';
...
class LocationController with ChangeNotifier {
  ...
  void getCurrentLocation(BuildContext context, bool fromAddress,
    ...
    try {
      ...
      // Only request permission if it's denied, not if it's already granted
      if (permission == LocationPermission.denied) {
        try {
          permission = await Geolocator.requestPermission();
        } on PlatformException catch (e) {
          // If another permission request is in progress, wait a bit and check again
          if (e.code == 'permissionRequestInProgress' ||
              e.message?.contains('permissionRequestInProgress') == true) {
            // Wait longer and retry checking permission
            ...
            if (kDebugMode) {
              print(
                  'Location permission request error: ${e.code} - ${e.message}');
            }
            return;
          }
        } catch (e) {
      ...
      Position myPosition;
      try {
        // Only call getCurrentPosition if we have permission
        ...
        // Re-throw if it's a different PlatformException
        if (kDebugMode) {
          print('Location getCurrentPosition error: ${e.code} - ${e.message}');
        }
        myPosition = Position(
      ...
      // Handle any other errors in permission checking
      _permissionInProgress = false;
      _loading = false;
      notifyListeners();
      return;
